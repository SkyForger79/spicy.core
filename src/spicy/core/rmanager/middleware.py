...
<html>
    ...
    <body>
        <form method='post' action='?_profile=1'>
...
def display_stats(request, stats, queries):
    ...
    _stats_ should contain a pstats.Stats of a profile session.
...
def display_queries(request, stats, queries):
    ...
    output.reset()
    output = [html.escape(line.decode('utf-8'))
...
class ProfileMiddleware(object):
    ...
    http://yoursite.com/yourview/?_profile=1
    """
	...
	"""
        ...
        if not (request.method == 'GET' or
                getattr(settings, 'FORCE_PROFILE', False)) and \
           not (request.META.get('HTTP_CONTENT_TYPE',
                                 request.META.get('CONTENT_TYPE', '')) in
                ['multipart/form-data', 'application/x-www-form-urlencoded']):
            return
        if ((request.GET.get('_profile', False) or
             getattr(settings, 'FORCE_PROFILE', False)
             ) and (settings.DEBUG == True or request.user.is_staff)):
            signals.request_started.disconnect(reset_queries)
            request.statsfile = tempfile.NamedTemporaryFile(suffix='.profile')
    ...
    def process_response(self, request, response):
        ...
        if profiler:
            ...
            stats.load_stats(request.statsfile.name)
