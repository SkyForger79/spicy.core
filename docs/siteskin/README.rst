...
Данная команда идентична встроенной Django-команде collectstatic
Она собирает статические файлы со всех ваших приложений (и остальных мест, которые вы укажите) в одном месте, 
что позволяет легко настроить выдачу статических файлов на боевом сервере.
...
Включить вывод 404 ошибок в консоли, по умолчанию выключен - значение False ::
...
Версия API для ajax-запросов, значение по умолчанию ``'0.1-default'``::

  AJAX_API_VERSION = 'your API version'
...
URL, по которому будет доступна главная страница темы. По умолчанию значение ``'index'``, т.е. кроме адреса http://localhost, главная страница будет доступна и по http://localhost/index/ ::

  SITESKIN_INDEX_VIEW = 'your index page url'  
  
Имя бэкенда для кэширования, значение по умолчанию - ``'default'``. Подробнее про кэширование в `документации Django <https://djbook.ru/rel1.4/topics/cache.html>`_ ::

  SITESKIN_CACHE_BACKEND = 'your cache backend'  
...
Количество объектов на странице для `пажинаторов spicy.core.siteskin <../../src/spicy/utils/filters.py>`_, по умолчанию 50::

  OBJECTS_PER_PAGE = 50
  
Количество кнопок страниц, которые будут видны в пажинаторе в начале, по умолчанию - 2. Т.е. если в пажинаторе более двух страниц, то будут видны кнопки первой, второй и далее троеточие::
  
  PAGES_FROM_START = 2
  
Количество кнопок страниц, которые будут видны в пажинаторе в конце, по умолчанию - 2. Например, если в пажинаторе 10 страниц страниц, то будут видны кнопки девятой и десятой, а перед ними троеточие::
...
URL до файла sitemap. По умолчанию значение ``''``. Указанное значение ``SITEMAP_URL`` будет подставлено в URL вида ``http://<site domain>/<SITEMAP_URL><file_prefix>sitemap_<file_number>.xml.gz``::
...
Формат даты и времени, который будет использован в файле sitemap. По умолчанию ``'%Y-%m-%dT%H:%M:%S+03:00'`` ::

  DATETIME_FORMAT = 'your formatting' 

  
Настройка поля SanitizingCharField
++++++++++++++++++++++++++++++++++
Использование ``SanitizingCharField`` из spicy.core.siteskin позволяет валидировать текст, использующий html разметку, например, тело статьи. Это поле проверяет допустимые DOM-элементы, аттрибуты, классы, устраняет лишние пробелы, и скрывает некорректные теги.
  
Включить использование поля ``SanitizingCharField``. Значение по умолчанию False - используется поле Django ``forms.CharField``::

  USE_SANITIZER = True
  
Указать, какие html-классы поле будет рассматривать как корректные. Значение по умолчанию ``{}`` ::
