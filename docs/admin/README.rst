...
Назначение
==========
spicy.core.admin является модулем spicy.core, предназначен для отображения административной части приложения. Его отличие от стандартной админки Django заключается в том, что в совокупности с другими модулями SpicyCMS позволяет формировать интерфейс под нужды конечного пользователя - редактора сайта, администратора и т.д. Также spicy.core.admin позволяет настраивать robots.txt, подключать и отключать другие модули SpicyCMS через админку и другое (подробнее см. в разделе "Для администратора сайта").
...
{TODO структура папок шаблонов, переменные контекстного процессора, шаблонные теги}
