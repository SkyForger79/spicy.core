...
@register.simple_tag
def nav_order_url(nav_filter_object, field, title, 
                     style_common='icon-sort', style_desc='icon-sort-down', style_asc='icon-sort-up'):
    """

    :param nav_filter_object: class ``spicy.core.siteskin.navigation.Navigation``
...
def nav_filter_url(nav_filter_object, field, value, title,   extra_style=''):
    ...
    if qd.get(field) == value:
        ...
        return '<a href="{0}" class="active">{1}</a>'.format(url, unicode(title).encode('utf-8'))
    else:
        qd[field] = value
        url = request.path + '?' + qd.urlencode()
